...
usage() {
  cat <<'EOF'
Usage: ./scripts/import-metabase-data.sh [options]

Options:
  --archive-path <path>   Input tar.gz archive path (default: artifacts/metabase_data.tar.gz)
  ...
  --no-stop               Do not stop/restart metabase during import
...
if [[ ! -f "$RESOLVED_ARCHIVE_PATH" ]]; then
  echo "Archive file '$RESOLVED_ARCHIVE_PATH' was not found." >&2
  exit 1
fi
...
if [[ "$NO_STOP" == "false" && "$METABASE_WAS_RUNNING" == "true" ]]; then
  echo "Stopping Metabase before import..."
...
echo "Importing '$RESOLVED_ARCHIVE_PATH' into volume '$VOLUME_NAME'..."
docker run --rm \
  -v "$VOLUME_NAME:/to" \
  --mount "type=bind,source=$ARCHIVE_DIR,target=/backup" \
  alpine sh -c "rm -rf /to/* /to/.[!.]* /to/..?* && tar xzf /backup/$ARCHIVE_FILE -C /to"

echo "Metabase data import completed for volume '$VOLUME_NAME'."
